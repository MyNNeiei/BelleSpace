...
class AppointmentDetailForm(ModelForm):
    class Meta:
        ...
        labels = {
            'staff_id': 'พนักงาน',  # Change label for staff_id to "พนักงาน"
        }  
    def clean(self):
        cleaned_data = super().clean()
...
class AppointmentForm(ModelForm):
    class Meta:
        model = Appointment
        fields = ['category', 'service', 'appointment_date' ]  # Include the required fields
        ...
        widgets = {
            'appointment_date': DateTimeInput(attrs={
                ...
                'class': 'p-2 rounded-md ml-6 text-xl',  # Custom styles
            }),
    ...
    # Update to ModelMultipleChoiceField for selecting multiple services
    service = forms.ModelMultipleChoiceField(
        queryset=Service.objects.none(),
        label="บริการ",
        widget=forms.SelectMultiple(attrs={"class": "form-control px-20 rounded-md ml-6 text-xl"})  # Enable multi-select
    )
